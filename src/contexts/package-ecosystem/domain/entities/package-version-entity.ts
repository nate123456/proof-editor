...
export interface PackageVersionEntityData {
...
export class PackageVersionEntity {
  private constructor(private readonly data: PackageVersionEntityData) {}

  static create(data: PackageVersionEntityData): Result<PackageVersionEntity, InvalidPackageVersionError> {
    if (!data.version.trim()) {
      return Result.failure(new InvalidPackageVersionError('Version cannot be empty'));
    }

    const versionConstraintResult = VersionConstraint.create(data.version);
    if (!versionConstraintResult.success) {
      return Result.failure(new InvalidPackageVersionError(
    ...
    if (data.status === 'deprecated' && !data.deprecationReason) {
      return Result.failure(new InvalidPackageVersionError(
        'Deprecated version must have deprecation reason'
      ));
    }
    ...
    return Result.success(new PackageVersionEntity(data));
  }
  ...
  static createNew(
    ...
    publishedAt?: Date
  ): Result<PackageVersionEntity, InvalidPackageVersionError> {
    const status = this.determineVersionStatus(version);
    
    const data: PackageVersionEntityData = {
      packageId,
    ...
    return PackageVersionEntity.create(data);
  }
  ...
  compareVersion(other: PackageVersionEntity): number {
    return this.compareVersionStrings(this.data.version, other.data.version);
  }

  isNewerThan(other: PackageVersionEntity): boolean {
    return this.compareVersion(other) > 0;
  }

  isOlderThan(other: PackageVersionEntity): boolean {
    return this.compareVersion(other) < 0;
  }

  isSameVersion(other: PackageVersionEntity): boolean {
    return this.compareVersion(other) === 0;
  }

  withStatus(status: VersionStatus, deprecationReason?: string): Result<PackageVersionEntity, InvalidPackageVersionError> {
    if (status === 'deprecated' && !deprecationReason) {
      return Result.failure(new InvalidPackageVersionError(
        'Deprecated status requires deprecation reason'
      ));
    }
    ...
    const updatedData = {
      ...this.data,
      status,
      deprecationReason: status === 'deprecated' ? deprecationReason : undefined
    };

    return PackageVersionEntity.create(updatedData);
  }

  withCompatibilityNotes(notes: string): Result<PackageVersionEntity, InvalidPackageVersionError> {
    const updatedData = {
      ...this.data,
      compatibilityNotes: notes.trim() || undefined
    };

    return PackageVersionEntity.create(updatedData);
  }

  withBreakingChanges(changes: string[]): Result<PackageVersionEntity, InvalidPackageVersionError> {
    const updatedData = {
      ...this.data,
  ...
  withMigrationGuide(guide: string): Result<PackageVersionEntity, InvalidPackageVersionError> {
    const updatedData = {
      ...this.data,
      migrationGuide: guide.trim() || undefined
    };

    return PackageVersionEntity.create(updatedData);
  }

  markAsDeprecated(reason: string): Result<PackageVersionEntity, InvalidPackageVersionError> {
    return this.withStatus('deprecated', reason);
  }

  markAsRemoved(): Result<PackageVersionEntity, InvalidPackageVersionError> {
    return this.withStatus('removed');
  }

  markAsStable(): Result<PackageVersionEntity, InvalidPackageVersionError> {
  ...
  equals(other: PackageVersionEntity): boolean {
    return (
