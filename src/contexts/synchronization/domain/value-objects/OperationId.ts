...
export class OperationId {
  ...
  static create(value: string): Result<OperationId, Error> {
    if (!value || typeof value !== 'string') {
      return { success: false, error: new Error('Operation ID must be a non-empty string') };
    }

    const trimmedValue = value.trim();
    if (trimmedValue.length === 0) {
      return { success: false, error: new Error('Operation ID cannot be empty or whitespace') };
    }

    if (trimmedValue.length > 128) {
      return { success: false, error: new Error('Operation ID cannot exceed 128 characters') };
    }

    if (!/^[a-zA-Z0-9-_:.]+$/.test(trimmedValue)) {
      return { success: false, error: new Error('Operation ID can only contain alphanumeric characters, hyphens, underscores, colons, and periods') };
    }

    return {
      success: true,
      data: new OperationId(trimmedValue)
    };
  }

  static generate(deviceId: DeviceId, sequenceNumber: number): Result<OperationId, Error> {
    if (sequenceNumber < 0) {
      return { success: false, error: new Error('Sequence number cannot be negative') };
    }
