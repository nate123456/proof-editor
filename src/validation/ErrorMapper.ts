...
export class ErrorMapper {
  static convertParseErrorToDiagnostic(
    error: ParseError,
  ...
  private static getErrorSeverity(errorType: ParseErrorType): vscode.DiagnosticSeverity {
    switch (errorType) {
  ...
  private static getErrorRange(error: ParseError, document: vscode.TextDocument): vscode.Range {
  ...
  private static getDisplayMessage(error: ParseError): string {
    ...
    const contextualHelp = this.getContextualHelp(error.type);
    if (contextualHelp) {
  ...
  private static getContextualHelp(errorType: ParseErrorType): string {
    switch (errorType) {
      case ParseErrorType.YAML_SYNTAX:
